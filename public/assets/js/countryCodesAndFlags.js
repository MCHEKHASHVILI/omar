...
country_code = document.querySelector('#countryCode');
...
/*window.addEventListener('DOMContentLoaded', function() {
  var input = document.querySelector('input[type="countryCode"]');
  var flag = document.querySelector('.iconify');
  ...
  fetch('https://api.ipify.org/?format=json')
    ...
    .then(data => {
      ...
      // Get country phone code based on IP address
      fetch('https://ipapi.co/' + ipAddress + '/country_calling_code/')
        .then(response => response.text())
        .then(countryCode => {
          // Set the input value to the country phone code
          input.value = countryCode;
          
          // Set the flag icon according to the country code
          //flag.setAttribute('data-icon', 'flag:' + countryCode + '-4x3');
          flag.classList.add('iconify');
        })
        .catch(error => {
...
window.addEventListener('DOMContentLoaded', function() {
document.querySelector("input[type='countryCode']").value='+44'
